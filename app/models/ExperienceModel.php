...
class ExperienceModel
{
	...
	{
		$query = DB::run("SELECT * FROM experiences LEFT JOIN countries ON experiences.country_id = countries.id WHERE experiences.id = :id", [':id' => $id]);
		$rows = mysqli_fetch_all($query, MYSQLI_ASSOC);
		foreach ($rows as $key => $value) {
			$query = DB::run("SELECT * FROM cities WHERE cities.country_id = :country_id", [':country_id' => $value['country_id']]);
			$records = mysqli_fetch_all($query, MYSQLI_ASSOC);
		...
		$query_cities = DB::run("SELECT * FROM cities WHERE country_id = :country_id", [':country_id' => $rows[0]['country_id']]);
